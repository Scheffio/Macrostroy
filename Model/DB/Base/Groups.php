...
abstract class Groups implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\DB\\Map\\GroupsTableMap';
    ...
    /**
     * Initializes internal state of DB\Base\Groups object.
     * @see applyDefaults()
     */
    ...
    public function setIsAvailable($v)
    {
        ...
        if ($this->is_available !== $v) {
            $this->is_available = $v;
            $this->modifiedColumns[GroupsTableMap::COL_IS_AVAILABLE] = true;
        }
    ...
    public function setSubprojectId($v)
    {
        ...
        if ($this->subproject_id !== $v) {
            ...
            $this->modifiedColumns[GroupsTableMap::COL_SUBPROJECT_ID] = true;
        }
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : GroupsTableMap::translateFieldName('Status', TableMap::TYPE_PHPNAME, $indexType)];
            $this->status = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : GroupsTableMap::translateFieldName('IsAvailable', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    {
        if ($this->aSubproject !== null && $this->subproject_id !== $this->aSubproject->getId()) {
            $this->aSubproject = null;
        }
    }
            ...
            if ($this->housesScheduledForDeletion !== null) {
                if (!$this->housesScheduledForDeletion->isEmpty()) {
                    \DB\HouseQuery::create()
                        ->filterByPrimaryKeys($this->housesScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->housesScheduledForDeletion = null;
                }
            }
    ...
    protected function doInsert(ConnectionInterface $con): void
    {
        ...
        if ($this->isColumnModified(GroupsTableMap::COL_IS_AVAILABLE)) {
    ...
    public function buildCriteria(): Criteria
    {
        ...
        if ($this->isColumnModified(GroupsTableMap::COL_IS_AVAILABLE)) {
            $criteria->add(GroupsTableMap::COL_IS_AVAILABLE, $this->is_available);
        }
        if ($this->isColumnModified(GroupsTableMap::COL_SUBPROJECT_ID)) {
     ...
     * objects.
     *
     * @param object $copyObj An object of \DB\Groups (or compatible) type.
     ...
     *
     * @param bool $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \DB\Groups Clone of current object.
     * @throws \Propel\Runtime\Exception\PropelException
     */
    ...
    {
        if ($this->aSubproject === null && ($this->subproject_id != 0)) {
            $this->aSubproject = ChildSubprojectQuery::create()->findPk($this->subproject_id, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aSubproject->addGroupss($this);
             */
        }
    ...
    {
        if (null !== $this->aSubproject) {
            $this->aSubproject->removeGroups($this);
        }
        $this->id = null;
        $this->name = null;
        $this->status = null;
