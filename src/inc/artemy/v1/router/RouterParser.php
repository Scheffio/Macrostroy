...
namespace inc\artemy\v1\router;
...
//    }
    ...
    public static function getUrlCatalog($number): ?string
    {
        $path = parse_url($_SERVER["REQUEST_URI"], PHP_URL_PATH);
        $path = explode("/", $path);

        return (!empty($path[$number]) and !str_contains($path[$number], ".")) ? $path[$number] : null;
    }
    ...
    {
        return RouterParser::getRootDirectory()
               . DIRECTORY_SEPARATOR
               . "src"
               . DIRECTORY_SEPARATOR
               . "public"
