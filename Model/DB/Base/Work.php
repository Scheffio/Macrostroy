...
abstract class Work implements ActiveRecordInterface
{
    ...
    {
        /** @var null|ChildWorkVersion[] $workVersionsToDelete */
    ...
    public function isVersioningNecessary(?ConnectionInterface $con = null): bool
    {
        ...
        if (ChildWorkQuery::isVersioningEnabled() && ($this->isNew() || $this->isModified()) || $this->isDeleted()) {
            return true;
        }
        if ($this->collWorkMaterials) {
     ...
     * @param ConnectionInterface $con The ConnectionInterface connection to use.
     *
     * @return ChildWorkVersion A version object
     */
    public function addVersion(?ConnectionInterface $con = null)
    {
        $this->enforceVersion = false;

        $version = new ChildWorkVersion();
    ...
    public function toVersion($versionNumber, ?ConnectionInterface $con = null)
    {
        ...
        if (!$version) {
            throw new PropelException(sprintf('No ChildWork object found with version %d', $version));
        }
     ...
     * Sets the properties of the current object to the value they had at a specific version
     *
     * @param ChildWorkVersion $version The version object to use
    ...
    {
        $loadedObjects['ChildWork'][$version->getId()][$version->getVersion()] = $this;
    ...
    {
        $v = ChildWorkVersionQuery::create()
            ->filterByWork($this)
            ->orderByVersion('desc')
            ->findOne($con);
        if (!$v) {
    ...
    {
        return ChildWorkVersionQuery::create()
            ->filterByWork($this)
     ...
     * @param ConnectionInterface $con The ConnectionInterface connection to use.
     *
     * @return ObjectCollection|ChildWorkVersion[] A list of ChildWorkVersion objects
     */
    public function getAllVersions(?ConnectionInterface $con = null)
    {
        $criteria = new Criteria();
        $criteria->addAscendingOrderByColumn(WorkVersionTableMap::COL_VERSION);

        return $this->getWorkVersions($criteria, $con);
    }
     ...
     * @param ConnectionInterface $con The ConnectionInterface connection to use.
     *
     * @return PropelCollection|\DB\WorkVersion[] List of \DB\WorkVersion objects
     */
    public function getLastVersions($number = 10, $criteria = null, ?ConnectionInterface $con = null)
    {
        $criteria = ChildWorkVersionQuery::create(null, $criteria);
        $criteria->addDescendingOrderByColumn(WorkVersionTableMap::COL_VERSION);
